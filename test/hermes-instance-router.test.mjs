...
import { buildHermesRouterRouteRecord } from '../server-lib/hermes-router-helpers.mjs'
...
test('实例路由服务会把 HTTP 请求转发到对应实例端口', async () => {
  const routesDir = createTempDirectory('hermes-router-routes-')
...
test('实例路由服务支持 WebSocket upgrade 转发', async () => {
  const routesDir = createTempDirectory('hermes-router-ws-routes-')
...
test('实例路由服务可选要求内部共享令牌', async () => {
  const routesDir = createTempDirectory('hermes-router-token-routes-')
  ...
  try {
    ...
    const invalidTokenResponse = await fetch(`http://127.0.0.1:${router.port}/instances/demo-token-instance/`, {
      headers: {
        'x-hermes-router-token': 'wrong-secret',
      },
    })
    ...
    const validTokenResponse = await fetch(`http://127.0.0.1:${router.port}/instances/demo-token-instance/`, {
      headers: {
        'x-hermes-router-token': 'router-secret',
      },
    })
