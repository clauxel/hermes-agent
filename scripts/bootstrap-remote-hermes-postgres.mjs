...
function getConfig(projectRoot) {
  ...
  if (devEnvironment.HERMES_DEPLOY_HOST !== prodEnvironment.HERMES_DEPLOY_HOST) {
  ...
  return {
    sshHost: devEnvironment.HERMES_DEPLOY_HOST,
    sshPort: Number.parseInt(devEnvironment.HERMES_DEPLOY_PORT ?? '22', 10) || 22,
    sshUsername: devEnvironment.HERMES_DEPLOY_USERNAME || 'root',
    sshPassword: devEnvironment.HERMES_DEPLOY_ROOT_PASSWORD || '',
    dev: {
      host: devEnvironment.HERMES_POSTGRES_HOST || '127.0.0.1',
      database: devEnvironment.HERMES_POSTGRES_DB || 'hermes_dev',
      user: devEnvironment.HERMES_POSTGRES_USER || 'hermes_dev',
      password: devEnvironment.HERMES_POSTGRES_PASSWORD || '',
      port: Number.parseInt(devEnvironment.HERMES_POSTGRES_PORT ?? '5432', 10) || 5432,
    },
    prod: {
      host: prodEnvironment.HERMES_POSTGRES_HOST || '127.0.0.1',
      database: prodEnvironment.HERMES_POSTGRES_DB || 'hermes_prod',
      user: prodEnvironment.HERMES_POSTGRES_USER || 'hermes_prod',
      password: prodEnvironment.HERMES_POSTGRES_PASSWORD || '',
      port: Number.parseInt(prodEnvironment.HERMES_POSTGRES_PORT ?? '5432', 10) || 5432,
    },
  }
}
...
TMP_SQL=/tmp/hermes-dev-prod-bootstrap.sql
...
if (!config.sshPassword) {
  throw new Error('Missing HERMES_DEPLOY_ROOT_PASSWORD in .env.development/.env.production.')
}

if (!config.dev.password || !config.prod.password) {
  throw new Error('Missing HERMES_POSTGRES_PASSWORD in .env.development or .env.production.')
}
