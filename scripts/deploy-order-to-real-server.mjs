...
function buildInstanceName(ownerName, modelId, channelId) {
  const base = slugify(`${ownerName}-${modelId}-${channelId}`) || 'hermes'
...
async function upsertDeploymentRecords(database, {
  ...
  const existingInstance = await database.prepare(`SELECT * FROM claw_instances WHERE deployment_id = ?`).get(deploymentId)
...
const tokenEncryptionSecret = process.env.HERMES_TOKEN_SECRET ?? `${projectRoot}:${databaseIdentity}`
const configEncryptionSecret = process.env.HERMES_CONFIG_SECRET ?? tokenEncryptionSecret
...
try {
  ...
  const instanceName = buildInstanceName(owner.name ?? 'hermes-guest', order.model_id, order.channel_id)
  ...
  const statusMessage = hasSavedToken
    ? 'Hermes console is ready. The saved channel token and model preset were written into the instance.'
    : 'Hermes console is ready. The model preset was written into the instance; bind a channel token later when you are ready.'
