...
function isLocalPackageTarget(host, environment = process.env) {
  if (String(environment.HERMES_DEPLOY_RUN_LOCAL ?? '').trim().toLowerCase() === 'true') {
    return true
  }
...
function defaultTemplateArchivePath() {
  return '/root/hermes-templates/hermes-template.tar.gz'
}
...
export async function findRemotePackageTarget({ database, query }) {
  ...
  if (!normalizedQuery) {
    throw new Error('Missing instance query. Provide an instance name such as hermes-guest-gpt-5-4-telegram-c8894e.')
  }
  ...
  if (!target) {
    throw new Error(`Unable to find a deployed Hermes instance matching "${normalizedQuery}".`)
  }
...
export function resolveRemotePackageRequest({
  ...
  const resolvedArchivePath =
    archivePath?.trim() ||
    deploymentConfig.hermes.archivePath ||
  ...
  if (!sshHost) {
    throw new Error('Missing SSH host. Set HERMES_DEPLOY_HOST or keep a target_server on the instance record.')
  }
  ...
  if (!runLocal && !deploymentConfig.server.password && !deploymentConfig.server.privateKey) {
    throw new Error(
      'Missing SSH credentials. Set HERMES_DEPLOY_ROOT_PASSWORD or HERMES_DEPLOY_PRIVATE_KEY_PATH before packaging.',
    )
  }
...
if [ ! -d "$APP_PATH" ]; then
  echo "Hermes app directory not found at $APP_PATH" >&2
  exit 1
fi
...
export async function packageRemoteHermesInstance({
  ...
  const configPath = resolve(projectRoot, 'hermes-agent.config.json')
  ...
  const encryptionSecret = environment.HERMES_CONFIG_SECRET ?? environment.HERMES_TOKEN_SECRET ?? `${projectRoot}:${identity}`
