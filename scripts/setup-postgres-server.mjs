...
export function loadPostgresSetupConfig(environment = process.env) {
  const sshHost = firstDefined(
    environment.HERMES_DEPLOY_HOST,
    environment.HERMES_SERVER_IP,
    environment.HERMES_SERVER_HOST,
  )
  const sshPassword = firstDefined(
    environment.HERMES_DEPLOY_ROOT_PASSWORD,
    environment.HERMES_ROOT_PASSWORD,
    environment.HERMES_DEPLOY_PASSWORD,
  )
  const databasePassword = firstDefined(environment.HERMES_POSTGRES_PASSWORD)

  if (!sshHost) {
    throw new Error(
      'Missing server host. Set HERMES_DEPLOY_HOST, HERMES_SERVER_IP, or HERMES_SERVER_HOST.',
    )
  }

  if (!sshPassword) {
    throw new Error(
      'Missing root password. Set HERMES_DEPLOY_ROOT_PASSWORD, HERMES_ROOT_PASSWORD, or HERMES_DEPLOY_PASSWORD.',
    )
  }

  if (!databasePassword) {
    throw new Error('Missing database password. Set HERMES_POSTGRES_PASSWORD.')
  }

  return {
    sshHost,
    sshPort: parseInteger(environment.HERMES_DEPLOY_PORT, 22),
    sshUsername: firstDefined(environment.HERMES_DEPLOY_USERNAME) || 'root',
    sshPassword,
    databaseName: firstDefined(environment.HERMES_POSTGRES_DB) || 'hermes_app',
    databaseUser: firstDefined(environment.HERMES_POSTGRES_USER) || 'hermes_app',
    databasePassword,
    databasePort: parseInteger(environment.HERMES_POSTGRES_PORT, 5432),
    allowedCidr: firstDefined(environment.HERMES_POSTGRES_ALLOWED_CIDR) || '0.0.0.0/0',
  }
}
...
export function renderHelpText() {
  return [
    ...
    'HERMES_DEPLOY_HOST or HERMES_SERVER_IP',
    'HERMES_DEPLOY_ROOT_PASSWORD or HERMES_ROOT_PASSWORD',
    'HERMES_POSTGRES_PASSWORD',
    '',
    'Optional environment variables:',
    'HERMES_DEPLOY_PORT=22',
    'HERMES_DEPLOY_USERNAME=root',
    'HERMES_POSTGRES_DB=hermes_app',
    'HERMES_POSTGRES_USER=hermes_app',
    'HERMES_POSTGRES_PORT=5432',
    'HERMES_POSTGRES_ALLOWED_CIDR=0.0.0.0/0',
  ].join('\n')
}
