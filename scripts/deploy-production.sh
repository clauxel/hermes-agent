...
usage() {
  cat <<'USAGE'
Deploy Hermes Agent to the production server.
...
Environment overrides:
  ...
  DEPLOY_KEY         default: /Users/xiamu/.ssh/hermesLaunch_prod_205_key
  APP_ROOT           default: /data/hermes-agent
  SERVICE_NAME       default: hermes-agent.service
...
DEPLOY_KEY="${DEPLOY_KEY:-/Users/xiamu/.ssh/hermesLaunch_prod_205_key}"
APP_ROOT="${APP_ROOT:-/data/hermes-agent}"
SERVICE_NAME="${SERVICE_NAME:-hermes-agent.service}"
RELEASE_ID="${RELEASE_ID:-$(date +%Y%m%d-%H%M%S)}"
ARCHIVE_NAME="hermes-agent-${RELEASE_ID}.tar.gz"
LOCAL_ARCHIVE="${TMPDIR:-/tmp}/${ARCHIVE_NAME}"
REMOTE_ARCHIVE="/tmp/${ARCHIVE_NAME}"
REMOTE_SCRIPT="/tmp/hermes-agent-deploy-${RELEASE_ID}.sh"
SSH_OPTS=(
...
ENV_FILE="$APP_ROOT/hermes-agent.env"
...
LOCAL_REMOTE_SCRIPT="${TMPDIR:-/tmp}/hermes-agent-deploy-${RELEASE_ID}.sh"
