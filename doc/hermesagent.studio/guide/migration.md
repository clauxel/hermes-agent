...
source: https://hermesagent.studio/guide/migration
mirrored_at: 2026-04-12T01:11:32Z
---
# Migration from Hermes

If you previously used the legacy Hermes framework, you can migrate to Hermes Agent with the built-in migration tooling in just a few minutes.

## Migration Overview

The `hermes claw migrate` command reads your legacy Hermes data directory, converts supported data formats, and writes them into Hermes Agent's layout.

### Supported Data Types

| Data type | Legacy Hermes location | Hermes Agent location |
| --- | --- | --- |
| Personas | `~/.hermes/personas/` | `~/.hermes/personas/` |
| Memory entries | `~/.hermes/memory/` | `~/.hermes/memory/` |
...
The default source directory is `~/.hermes`. If your old Hermes data lives somewhere else, pass it with `--source`.
...
You can also open an issue on [GitHub Issues](https://github.com/clauxel/Hermes-Agent/issues). If you attach `migration_report.txt`, make sure you remove secrets first.
