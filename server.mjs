...
import { loadDeploymentConfig, readConfiguredHermesRepoRef } from './server-lib/deployment-config.mjs'
import {
...
const explicitEnvironmentPath = String(process.env.HERMES_ENV_PATH ?? '').toLowerCase()
...
const guestCookieName = 'hms_guest'
const hermesConsoleSessionCookieName = 'hms_console_session'
const sessionTtlMs = 1000 * 60 * 60 * 24 * 7
const guestTtlMs = 1000 * 60 * 60 * 24 * 30
const hermesConsoleContextTtlMs = 1000 * 60 * 60 * 12
...
const hermesConsoleSessions = new Map()
const {
  database,
...
const tokenEncryptionKey = createHash('sha256')
  .update(process.env.HERMES_TOKEN_SECRET ?? `${projectRoot}:${databaseIdentity}`)
  .digest()
const deploymentConfigPath =
  process.env.HERMES_CONFIG_PATH ?? join(projectRoot, 'hermes-agent.config.json')
const deploymentCommandJson = process.env.HERMES_DEPLOY_COMMAND_JSON
  ? JSON.parse(process.env.HERMES_DEPLOY_COMMAND_JSON)
  : null
const deploymentCommand = process.env.HERMES_DEPLOY_COMMAND ?? ''
const deploymentTargetServer = process.env.HERMES_DEPLOY_TARGET ?? 'simulation-cluster'
const deploymentConsoleBaseUrl = process.env.HERMES_CONSOLE_BASE_URL ?? 'https://console.hermes.local'
const deploymentPublicBaseUrl = process.env.HERMES_PUBLIC_BASE_URL ?? 'https://hermes.local'
const hermesRouterSharedToken = String(process.env.HERMES_ROUTER_SHARED_TOKEN ?? '').trim()
const defaultHermesRepoRef = 'main'
const allowSimulatedDeployment = process.env.HERMES_ALLOW_SIMULATED_DEPLOYMENT !== 'false'
const guestUserEmail = 'guest@hermes.local'
const guestUserName = 'Hermes Guest'
...
const serverSeoPageMap = new Map([
  [
    '/',
    {
      title: 'Hermes Agent | A Persistent AI Agent That Remembers, Acts, and Improves',
  ...
  [
    ...
    {
      title: 'Hermes Agent vs a normal AI chatbot | Hermes Agent',
      description:
        'Compare Hermes Agent with a normal AI chatbot when the real question is memory, tool use, and multi-step follow-through.',
      robots: 'index,follow',
    },
  ],
  [
    ...
    {
      title: 'Hermes Agent vs a rigid workflow bot | Hermes Agent',
      description:
        'Compare Hermes Agent with a workflow bot when the work has branching logic, changing context, and exceptions.',
      robots: 'index,follow',
    },
  ],
  [
    ...
    {
      title: 'Hermes Agent as a personal agent | Hermes Agent',
      description:
        'See when Hermes Agent is the right fit as a personal agent for durable context, memory, and follow-through.',
      robots: 'index,follow',
    },
  ],
  [
    ...
    {
      title: 'Hermes Agent as an AI digital employee | Hermes Agent',
      description:
        'See when Hermes Agent fits the digital-employee framing for owned slices of recurring work.',
      robots: 'index,follow',
    },
  ],
  [
    ...
    {
      title: 'Hermes Agent for AI task automation | Hermes Agent',
      description:
        'See when Hermes Agent is the better fit for task automation that needs memory, tools, and multi-step execution.',
      robots: 'index,follow',
    },
  ],
  [
    '/privacy',
    {
      title: 'Privacy Policy | Hermes Agent',
      description:
  ...
  [
    '/terms',
    {
      title: 'Terms of Service | Hermes Agent',
      description:
  ...
  [
    '/plans',
    {
      title: 'Pricing Plans | Hermes Agent',
      description:
  ...
  [
    '/checkout',
    {
      title: 'Checkout | Hermes Agent',
      description:
        'Continue through payment and launch tracking inside the Hermes Agent checkout flow.',
      robots: 'noindex,nofollow',
    },
  ],
])
...
const findLatestClawInstanceByOrderIdStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  WHERE order_id = ?
  ORDER BY sequence_number DESC, created_at DESC
...
const listClawInstancesByOrderIdStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  WHERE order_id = ?
  ORDER BY sequence_number DESC, created_at DESC
...
const findClawInstanceByDeploymentIdStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  WHERE deployment_id = ?
`)
...
const createClawInstanceStatement = database.prepare(`
  INSERT INTO claw_instances (
    id,
    order_id,
    deployment_id,
    user_id,
...
const updateClawInstanceStatement = database.prepare(`
...
const updateClawUpgradeStatement = database.prepare(`
  UPDATE claw_instances
  SET
    hermes_version = ?,
    upgrade_status = ?,
...
const bindClawInstanceToUserStatement = database.prepare(`
  UPDATE claw_instances
  SET user_id = ?, updated_at = ?
  WHERE order_id = ?
`)

const findClawInstanceByOrderIdStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  WHERE order_id = ?
  ORDER BY sequence_number DESC, created_at DESC
...
const listClawInstancesByUserStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  WHERE user_id = ?
  ORDER BY created_at DESC
...
const listClawInstancesByGuestTokenStatement = database.prepare(`
  SELECT claw_instances.*
  FROM claw_instances
  INNER JOIN orders ON orders.id = claw_instances.order_id
  WHERE orders.guest_token = ?
  ORDER BY claw_instances.created_at DESC
`)

const listAllClawInstancesStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  ORDER BY created_at DESC
`)

const findClawInstanceByIdStatement = database.prepare(`
  SELECT *
  FROM claw_instances
  WHERE id = ?
`)

const deleteClawInstanceByIdStatement = database.prepare(`
  DELETE FROM claw_instances
  WHERE id = ?
`)
...
function getHermesProxyMatch(requestUrl) {
  return requestUrl.pathname.match(/^\/hermes-console\/session\/([a-f0-9]+)(\/.*)?$/)
}
...
function buildModelProxyUpstreamUrl(requestUrl, upstreamPath) {
  const upstreamBaseUrl = normalizeModelProxyUpstreamBaseUrl(process.env.HERMES_MODEL_PROXY_BASE_URL ?? '')
  if (!upstreamBaseUrl) {
    return null
  }
...
function getModelProxyChatMaxTokensCap() {
  const rawValue = String(process.env.HERMES_MODEL_PROXY_CHAT_MAX_TOKENS_CAP ?? '').trim()
  if (!rawValue) {
    return 0
  }
...
async function proxyModelRequest({ request, response, requestUrl, instanceName, upstreamPath }) {
  ...
  proxyHeaders['x-hermes-instance-name'] = instanceName
...
function createHermesConsoleSession({ orderId, deploymentId, consoleUrl, consoleToken }) {
  cleanupExpiredHermesConsoleSessions()

  const sessionId = randomBytes(16).toString('hex')
  const expiresAt = Date.now() + hermesConsoleContextTtlMs
  const session = {
    id: sessionId,
    orderId,
    deploymentId,
  ...
  hermesConsoleSessions.set(sessionId, session)
  return session
}

function getHermesConsoleSession(sessionId) {
  if (!sessionId) {
    return null
  }

  const session = hermesConsoleSessions.get(sessionId)
  if (!session) {
    return null
  }

  if (session.expiresAt <= Date.now()) {
    hermesConsoleSessions.delete(sessionId)
    return null
  }
...
function buildHermesConsoleSessionUrl({ sessionId, deploymentId, consoleToken, guestToken }) {
  const url = new URL(`/hermes-console/session/${sessionId}/`, 'http://localhost')
  if (deploymentId) {
...
function createHermesConsoleSessionUrl({ orderId, deploymentId, consoleUrl, consoleToken, guestToken }) {
  const session = createHermesConsoleSession({
    orderId,
    deploymentId,
    consoleUrl,
    consoleToken,
  ...
  return buildHermesConsoleSessionUrl({
...
function setHermesConsoleContextCookies(response, request, { sessionId, guestToken = null }) {
  const baseParts = [
    'Path=/hermes-console',
    `Max-Age=${Math.floor(hermesConsoleContextTtlMs / 1000)}`,
    'HttpOnly',
    'SameSite=Lax',
  ]
  ...
  appendSetCookieHeader(
    response,
    `${hermesConsoleSessionCookieName}=${encodeURIComponent(sessionId)}; ${baseParts.join('; ')}`,
  )
...
async function resolveHermesProxyTarget(request, overrideSessionId, overridePathSuffix) {
  const requestUrl = new URL(request.url ?? '/', `http://${request.headers.host ?? 'localhost'}`)
  const match = getHermesProxyMatch(requestUrl)
  ...
  const session = getHermesConsoleSession(sessionId)
  if (!session) {
    throw new HttpError(410, 'Hermes console session expired. Reopen it from the dashboard.')
  }
  ...
  if (!deployment || deployment.order_id !== order.id || !session.consoleUrl) {
    throw new HttpError(400, 'Hermes console is not ready yet.')
  }

  const claw = await findClawInstanceByDeploymentIdStatement.get(deployment.id)
  if (claw?.runtime_state === 'stopped') {
    throw new HttpError(400, 'Hermes is stopped. Start a new deployment or uninstall this instance first.')
  }
  ...
  return {
    basePath: `/hermes-console/session/${session.id}`,
...
function filterProxyRequestHeaders(headers, upstreamUrl) {
  ...
  if (hermesRouterSharedToken) {
    nextHeaders['x-hermes-router-token'] = hermesRouterSharedToken
  }

  return nextHeaders
}

function injectHermesConsoleTokenPersistence(html, consoleToken, basePath, deploymentId) {
  ...
  const bootstrap = [
    ...
    "const settingsPrefix='hermes.control.settings.v1';",
    "const deploymentPrefix='hermes.control.deployment.v1:';",
    ...
    "const extractConsoleSuffix=(value)=>{const pathname=String(getConsoleUrl(value).pathname||'');const match=pathname.match(/^\\/hermes-console\\/session\\/[a-f0-9]+(\\/.*)?$/);return match?match[1]||'':null;};",
    ...
    "const clearStaleConsoleState=()=>{removeMatchingKeys(window.sessionStorage,(key)=>key==='hermes.control.settings.v1'||key.startsWith('hermes.control.token.v1:')||key.startsWith('hermes.control.deployment.v1:'));removeMatchingKeys(window.localStorage,(key)=>key==='hermes.control.settings.v1'||key.startsWith('hermes.control.settings.v1:')||key.startsWith('hermes.control.deployment.v1:'));};",
    ...
    "const readStoredToken=(storageGatewayUrl)=>{try{const directKey='hermes.control.token.v1:'+storageGatewayUrl;const directValue=window.sessionStorage.getItem(directKey);if(directValue)return directValue;for(let index=0;index<window.sessionStorage.length;index+=1){const key=window.sessionStorage.key(index);if(key&&key.startsWith('hermes.control.token.v1:')){const storedValue=window.sessionStorage.getItem(key);if(storedValue)return storedValue}}return window.__HERMES_PROXY_TOKEN||token}catch{return window.__HERMES_PROXY_TOKEN||token}};",
    "const readGuestToken=(value)=>{try{return getConsoleUrl(value).searchParams.get('guest_token')||''}catch{return ''}};",
    "const persistConsoleState=(value)=>{const nextGatewayUrl=resolveGatewayUrl(value);const nextStorageGatewayUrl=normalizeGatewayUrl(nextGatewayUrl.replace(/[?#].*$/,''));const nextSettingsStorageKey=settingsPrefix+':'+nextStorageGatewayUrl;const nextTokenStorageKey='hermes.control.token.v1:'+nextStorageGatewayUrl;const nextDeploymentStorageKey=deploymentPrefix+nextStorageGatewayUrl;const activeToken=readStoredToken(nextStorageGatewayUrl);try{const nextSettings={...readSettings(nextSettingsStorageKey),gatewayUrl:nextGatewayUrl};window.localStorage.setItem(settingsPrefix,JSON.stringify(nextSettings));window.localStorage.setItem(nextSettingsStorageKey,JSON.stringify(nextSettings))}catch{}if(activeToken){try{window.sessionStorage.setItem(nextTokenStorageKey,activeToken)}catch{}}if(deploymentId){try{window.sessionStorage.setItem(nextDeploymentStorageKey,deploymentId);window.localStorage.setItem(nextDeploymentStorageKey,deploymentId)}catch{}}window.__HERMES_PROXY_TOKEN=activeToken;return {gatewayUrl:nextGatewayUrl,storageGatewayUrl:nextStorageGatewayUrl};};",
    ...
    "window.__HERMES_CONTROL_UI_BASE_PATH__=basePath;",
    ...
    'window.__HERMES_PROXY_TOKEN=readStoredToken(consoleState.storageGatewayUrl);',
    '})();</script>',
  ].join('')
  return html.replace('</head>', `${bootstrap}</head>`)
}

function getHermesProxyRetryDelays(method) {
...
async function fetchHermesProxyUpstream(target, request, method, body) {
  const retryDelays = getHermesProxyRetryDelays(method)
  let lastError = null
  ...
  throw lastError ?? new Error('Hermes console upstream request failed.')
}

function getHermesProxyUpgradeRetryDelays() {
  return [0, 250, 500, 1000]
}

async function handleHermesConsoleProxyRequest(request, response) {
  let target

  try {
    target = await resolveHermesProxyTarget(request)
  } catch (error) {
    const requestUrl = new URL(request.url ?? '/', `http://${request.headers.host ?? 'localhost'}`)
  ...
  if (!requestUrl.pathname.endsWith('/') && target.pathSuffix === '/') {
    setHermesConsoleContextCookies(response, request, {
  ...
  try {
    upstreamResponse = await fetchHermesProxyUpstream(
      target,
      request,
      method,
    ...
    throw new HttpError(502, 'Hermes console is temporarily unreachable.')
  }
  ...
  if (contentType.includes('text/html')) {
    ...
    payload = Buffer.from(
      injectHermesConsoleTokenPersistence(html, target.consoleToken, target.basePath, target.deploymentId),
      'utf8',
    )
  }
...
async function handleHermesConsoleProxyUpgrade(request, socket, head) {
  let target
  ...
  try {
    ...
    const match = getHermesProxyMatch(urlObj)
    const cookies = parseCookies(request)

    try {
      target = await resolveHermesProxyTarget(request)
      if (!target) {
        throw new HttpError(404, 'Hermes console session route not found.')
      }
    } catch (error) {
      ...
      if (refererUrl) {
        const refererMatch = getHermesProxyMatch(refererUrl)
        if (refererMatch) {
      ...
      if (!overrideSessionId) {
        const contextSessionId = cookies[hermesConsoleSessionCookieName]
        if (contextSessionId) {
      ...
      target = await resolveHermesProxyTarget(request, overrideSessionId, overridePathSuffix)
    }
  ...
  const retryDelays = getHermesProxyUpgradeRetryDelays()
  let settled = false
  ...
  const startAttempt = (attemptIndex) => {
    ...
    upstreamRequest.setTimeout(5000, () => {
      upstreamRequest.destroy(new Error('Hermes websocket upstream timed out.'))
    })
...
function cleanupExpiredHermesConsoleSessions() {
  const now = Date.now()

  for (const [sessionId, session] of hermesConsoleSessions.entries()) {
...
function getDeploymentRuntimeConfig() {
  return loadDeploymentConfig({
    ...
    encryptionSecret:
      process.env.HERMES_CONFIG_SECRET ??
      process.env.HERMES_TOKEN_SECRET ??
      `${projectRoot}:${databaseIdentity}`,
  })
}

function getConfiguredHermesVersion() {
  try {
    return readConfiguredHermesRepoRef(deploymentConfigPath) || defaultHermesRepoRef
  } catch {
    return defaultHermesRepoRef
  }
}

function getClawHermesVersion(row) {
  if (typeof row?.hermes_version === 'string' && row.hermes_version.trim()) {
    return row.hermes_version.trim()
  }

  return getConfiguredHermesVersion()
}

function getClawUpgradeStatus(row) {
...
function buildOrderNumber() {
  return `OCL-${Date.now().toString().slice(-8)}`
}

function buildInstanceName(user, modelId, channelId) {
  const base = slugify(`${user.name}-${modelId}-${channelId}`) || 'hermes'
...
async function createDeploymentForOrder(order, triggerMode) {
  ...
  await updateOrderDeploymentStatus(
    ...
    'queued',
    triggerMode === 'manual'
      ? 'Manual deployment trigger accepted. A new Hermes workspace is queued.'
...
async function bindOrderToUserAccount(orderId, userId) {
  ...
  await bindClawInstanceToUserStatement.run(userId, timestamp, orderId)
  return await findOrderByIdStatement.get(orderId)
}

const { serializeClawInstance, serializeDeployment, serializeOrder, serializePlan } =
  createSerializationHelpers({
    annualBillingMultiplier,
    buildMaskedTokenDisplay,
    decryptSecretValue,
    findDeploymentByOrderIdStatement,
    findLatestClawInstanceByOrderIdStatement,
    formatMoney,
    getChannelById,
    getClawHermesVersion,
    getClawUpgradeStatus,
    getConfiguredHermesVersion,
    getModelById,
...
async function updateClawUpgradeState(clawId, patch) {
  const current = await findClawInstanceByIdStatement.get(clawId)
  if (!current) {
    throw new HttpError(404, 'Hermes workspace not found.')
  }

  await updateClawUpgradeStatement.run(
    patch.hermesVersion ?? current.hermes_version ?? null,
  ...
  return await findClawInstanceByIdStatement.get(clawId)
}

async function updateClawRuntimeState(clawId, runtimeState) {
  const current = await findClawInstanceByIdStatement.get(clawId)
  if (!current) {
    throw new HttpError(404, 'Hermes workspace not found.')
  }
...
async function upsertClawInstance({
  orderId,
  deploymentId,
  userId,
  sequenceNumber,
  ...
  const existing = await database.prepare(`SELECT * FROM claw_instances WHERE deployment_id = ?`).get(deploymentId)
  const timestamp = nowIso()

  if (existing) {
    await updateClawInstanceStatement.run(
      status,
      targetServer,
      workspacePath ?? null,
      consoleUrl ?? null,
      ...
      hermesVersion ?? existing.hermes_version ?? null,
      'idle',
      null,
      null,
      timestamp,
    ...
    return await database.prepare(`SELECT * FROM claw_instances WHERE deployment_id = ?`).get(deploymentId)
  }

  await createClawInstanceStatement.run(
    randomBytes(16).toString('hex'),
    orderId,
    deploymentId,
    userId,
...
async function listVisibleClawInstances(context) {
  const rows =
    context.kind === 'guest'
      ? await listClawInstancesByGuestTokenStatement.all(context.guestToken)
      : context.user.role === 'admin'
        ? await listAllClawInstancesStatement.all()
        : [
            ...(await listClawInstancesByUserStatement.all(context.user.id)),
            ...(context.guestToken ? await listClawInstancesByGuestTokenStatement.all(context.guestToken) : []),
          ]

  return Array.from(new Map(rows.map((row) => [row.id, row])).values()).map(serializeClawInstance)
}
...
async function executeDeployment(context) {
  ...
  if (Array.isArray(deploymentCommandJson) && deploymentCommandJson.length > 0) {
    ...
    return {
      ...
      workspacePath:
        parsed.workspacePath ?? `/srv/hermes/${context.instanceName}`,
      consoleUrl:
        parsed.consoleUrl ??
  ...
  return {
    ...
    workspacePath: `/srv/hermes/${context.instanceName}`,
...
async function processDeployment(deploymentId) {
  ...
  try {
    ...
    await updateOrderDeploymentStatus(order.id, 'provisioning', 'Deployment started. Preparing your Hermes workspace.')
    ...
    await updateDeploymentRecord(deploymentId, {
      progress: 55,
      lastMessage: 'Connecting to the deployment target and applying Hermes Agent config.',
      updatedAt: nowIso(),
    })
    ...
    await updateDeploymentRecord(deploymentId, {
      ...
      lastMessage: 'Deployment completed. Hermes Agent is ready in the console.',
      runLogs: deploymentResult.runLogs,
      finishedAt: completedAt,
      updatedAt: completedAt,
    })

    await upsertClawInstance({
      orderId: order.id,
      deploymentId,
      userId: user.id,
      ...
      runtimeUser: deploymentResult.runtimeUser,
      serviceName: deploymentResult.serviceName,
      hermesVersion: deploymentResult.hermesVersion,
    })

    await updateOrderDeploymentStatus(
      order.id,
      'deployed',
      'Deployment finished. Hermes Agent is live and visible in the console.',
    )
  } catch (error) {
    ...
    if (current) {
      ...
      const hermesVersion = errorDetails?.hermesVersion ?? current.hermes_version
      await updateDeploymentRecord(deploymentId, {
        status: 'failed',
...
async function upgradeHermesInstance(order, targetVersion) {
  if (order.payment_status !== 'paid') {
    throw new HttpError(400, 'Pay for this order before upgrading Hermes Agent.')
  }

  const claw = await findLatestClawInstanceByOrderIdStatement.get(order.id)
  if (!claw) {
    throw new HttpError(400, 'Hermes Agent is not deployed yet.')
  }

  if (!claw.workspace_path || !claw.runtime_user || !claw.service_name) {
    throw new HttpError(400, 'Hermes runtime metadata is incomplete. Re-deploy the workspace first.')
  }

  if (getClawUpgradeStatus(claw) === 'in_progress') {
    throw new HttpError(409, 'Hermes upgrade is already in progress.')
  }

  const normalizedVersion = String(targetVersion ?? '').trim()
  if (!normalizedVersion) {
    throw new HttpError(400, 'Choose a target Hermes version first.')
  }

  const config = getDeploymentRuntimeConfig()
  await updateClawUpgradeState(claw.id, {
    upgradeStatus: 'in_progress',
    upgradeTargetVersion: normalizedVersion,
    upgradeError: null,
    updatedAt: nowIso(),
  })
  await updateOrderDeploymentStatus(order.id, order.deployment_status, `Upgrading Hermes Agent to ${normalizedVersion}.`)
  ...
  try {
    ...
    await upsertClawInstance({
      ...
      hermesVersion: result.hermesVersion ?? normalizedVersion,
    })
    ...
    await updateOrderDeploymentStatus(order.id, order.deployment_status, `Hermes Agent upgraded to ${normalizedVersion}.`)
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Hermes upgrade failed.'
    await updateClawUpgradeState(claw.id, {
    ...
    await updateOrderDeploymentStatus(order.id, order.deployment_status, `Hermes upgrade failed: ${message}`)
    throw error
  }
...
async function refreshOrderDeploymentAfterInstanceRemoval(order) {
  ...
  if (latestDeployment) {
    await updateOrderDeploymentStatus(
      ...
      latestDeployment.last_message || 'Latest Hermes workspace record is available in the console.',
    )
    return
  }

  if (order.payment_status === 'paid') {
    await updateOrderDeploymentStatus(order.id, 'failed', 'Hermes workspace uninstalled. Create a new workspace to deploy again.')
    return
  }

  await updateOrderDeploymentStatus(order.id, 'pending_payment', 'Waiting for payment before Hermes Agent can be launched.')
}

async function stopHermesInstance(order) {
  if (order.payment_status !== 'paid') {
    throw new HttpError(400, 'Pay for this order before managing Hermes Agent.')
  }
  ...
  await updateClawRuntimeState(claw.id, result.runtimeState ?? 'stopped')
  await updateOrderDeploymentStatus(order.id, order.deployment_status, 'Hermes workspace stopped. You can still uninstall or redeploy it later.')

  return await findOrderByIdStatement.get(order.id)
}

async function uninstallHermesInstance(order) {
  if (order.payment_status !== 'paid') {
    throw new HttpError(400, 'Pay for this order before managing Hermes Agent.')
  }
  ...
  const clawsToRemove =
    claw.status === 'failed'
      ? await listClawInstancesByOrderIdStatement.all(order.id)
      : [claw]
  ...
  for (const item of clawsToRemove) {
    ...
    await deleteClawInstanceByIdStatement.run(item.id)
...
async function deletePendingOrder(order) {
  ...
  if ((await findDeploymentByOrderIdStatement.get(order.id)) || (await findLatestClawInstanceByOrderIdStatement.get(order.id))) {
...
const apiRouter = createApiRouter({
  ...
  findClawInstanceByDeploymentIdStatement,
...
const server = createServer(async (request, response) => {
  ...
  try {
    if (await handleHermesConsoleProxyRequest(request, response)) {
      return
    }
...
server.on('upgrade', (request, socket, head) => {
  void handleHermesConsoleProxyUpgrade(request, socket, head)
})

export const serverReady = new Promise((resolve) => {
  server.listen(port, () => {
    console.log(
      `Hermes Agent server running on http://localhost:${port} using ${databaseProvider} database ${databaseIdentity}`,
    )
    resolve()
  })
})
...
export async function stopHermesAgentServer() {
  if (didStopServer) {
    return
  }
...
function getServerSeoConfig(request, pathname) {
  ...
  return {
    title: 'Page not found | Hermes Agent',
    description:
...
function isHtmlPageRequest(request, requestUrl) {
  ...
  return !acceptHeader || acceptHeader === '*/*' || acceptHeader.includes('text/html')
}
...
function serveProductionAsset(request, response) {
  ...
  if (isHtmlPageRequest(request, requestUrl)) {
    ...
    response.setHeader('Cache-Control', 'no-store')
    response.setHeader('X-Robots-Tag', seo.robots)
