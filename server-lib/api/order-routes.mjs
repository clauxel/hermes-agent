  ...
  return [
    createExactRoute('GET', '/api/console-data', async ({ request, response }) => {
      ...
      sendJson(response, 200, {
        ...
        claws: await listVisibleClawInstances(context),
        users:
    ...
    createPatternRoute('POST', /^\/api\/orders\/([a-f0-9]+)\/hermes-console$/, async ({ request, response, params }) => {
      const context = await requireOrderAccessContext(request)
      ...
      const claw = requestedDeploymentId
        ? await findClawInstanceByDeploymentIdStatement.get(deployment.id)
        : await findLatestClawInstanceByOrderIdStatement.get(order.id)
      ...
      const consoleUrl = createHermesConsoleSessionUrl({
        orderId: order.id,
    ...
    createPatternRoute('POST', /^\/api\/orders\/([a-f0-9]+)\/hermes-stop$/, async ({ request, response, params }) => {
      const context = await requireOrderAccessContext(request)
      const order = await findOrderByIdStatement.get(params[1])
      await assertOrderAccess(context, order)

      const updatedOrder = await stopHermesInstance(order)
      sendJson(response, 200, {
        message: 'Hermes stopped successfully.',
        order: await serializeOrder(updatedOrder),
      })
    }),
    createPatternRoute('POST', /^\/api\/orders\/([a-f0-9]+)\/hermes-uninstall$/, async ({ request, response, params }) => {
      const context = await requireOrderAccessContext(request)
      const order = await findOrderByIdStatement.get(params[1])
      await assertOrderAccess(context, order)

      const updatedOrder = await uninstallHermesInstance(order)
      sendJson(response, 200, {
        message: 'Hermes uninstalled successfully.',
        order: await serializeOrder(updatedOrder),
      })
    }),
    createPatternRoute('POST', /^\/api\/orders\/([a-f0-9]+)\/hermes-delete$/, async ({ request, response, params }) => {
      const context = await requireOrderAccessContext(request)
      const order = await findOrderByIdStatement.get(params[1])
      await assertOrderAccess(context, order)
    ...
    createPatternRoute('POST', /^\/api\/orders\/([a-f0-9]+)\/deployments$/, async ({ request, response, params }) => {
      ...
      if (order.payment_status !== 'paid') {
        throw new HttpError(400, 'Pay for this order before triggering another Hermes deployment.')
      }

      if (!(await canTriggerDeployment(order))) {
        throw new HttpError(400, 'This order has already used every deployable Hermes slot.')
      }
    ...
    createPatternRoute('GET', /^\/api\/orders\/([a-f0-9]+)\/hermes-versions$/, async ({ request, response, params }) => {
      const context = await requireOrderAccessContext(request)
      const order = await findOrderByIdStatement.get(params[1])
      await assertOrderAccess(context, order)
      ...
      if (!claw) {
        throw new HttpError(400, 'Hermes is not deployed yet.')
      }

      const config = getDeploymentRuntimeConfig()
      const versions = await listConfiguredHermesVersions(config)

      sendJson(response, 200, {
        versions,
        currentVersion: getClawHermesVersion(claw),
        configuredVersion: config.hermes.repoRef,
      })
    }),
    createPatternRoute('POST', /^\/api\/orders\/([a-f0-9]+)\/hermes-upgrade$/, async ({ request, response, params }) => {
      const context = await requireOrderAccessContext(request)
      const order = await findOrderByIdStatement.get(params[1])
      await assertOrderAccess(context, order)

      const body = await readJsonBody(request)
      const upgradedOrder = await upgradeHermesInstance(order, body.version)

      sendJson(response, 200, {
        message: `Hermes has been upgraded to ${String(body.version ?? '').trim()}.`,
