...
import {
  buildHermesRouterRouteFilePath,
  normalizeHermesRouterInstanceName,
...
function resolveRouterRequestPath(pathname) {
  ...
  return {
    instanceName: normalizeHermesRouterInstanceName(decodeURIComponent(match[1] ?? '')),
...
function readRouteRecord(routesDir, instanceName) {
  const routeFilePath = buildHermesRouterRouteFilePath(routesDir, instanceName)
  if (!existsSync(routeFilePath)) {
    throw createRouterHttpError(404, 'Hermes instance route is not registered.')
  }
  ...
  try {
    ...
    throw createRouterHttpError(500, 'Hermes instance route metadata is invalid.')
  }

  const consolePort = Number.parseInt(String(parsed?.consolePort ?? ''), 10)
  if (!Number.isInteger(consolePort) || consolePort <= 0) {
    throw createRouterHttpError(500, 'Hermes instance route is missing a valid console port.')
  }
...
function validateRouterToken(request, sharedToken) {
  ...
  const providedToken = String(request.headers['x-hermes-router-token'] ?? '').trim()
  if (!providedToken) {
    throw createRouterHttpError(401, 'Hermes router authentication is required.')
  }

  if (providedToken !== sharedToken) {
    throw createRouterHttpError(403, 'Hermes router authentication failed.')
  }
}
...
async function proxyHttpRequest({ request, response, routesDir, sharedToken }) {
  ...
  if (!routeMatch) {
    jsonResponse(response, 404, { message: 'Hermes router route not found.' })
    return
  }

  const routeRecord = readRouteRecord(routesDir, routeMatch.instanceName)
  if (routeRecord.runtimeState !== 'running') {
    jsonResponse(response, 409, { message: 'Hermes instance is not running.' })
    return
  }
  ...
  try {
    ...
    throw createRouterHttpError(502, 'Hermes instance upstream is unreachable.')
  }
...
async function proxyUpgradeRequest({ request, socket, head, routesDir, sharedToken }) {
  try {
    ...
    if (!routeMatch) {
      ...
      socket.end('Hermes router route not found.')
      return
    }
    ...
    if (routeRecord.runtimeState !== 'running') {
      ...
      socket.end('Hermes instance is not running.')
      return
    }
    ...
    const message = error instanceof Error ? error.message : 'Hermes router failed.'
...
export function createHermesInstanceRouter({
  routesDir = process.env.HERMES_ROUTER_ROUTES_DIR ?? '/data/hermes/router/routes',
  host = process.env.HERMES_ROUTER_HOST ?? '127.0.0.1',
  port = Number(process.env.HERMES_ROUTER_PORT ?? 19080),
  sharedToken = process.env.HERMES_ROUTER_SHARED_TOKEN ?? '',
} = {}) {
  const router = {
    host,
    port,
