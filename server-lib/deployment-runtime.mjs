...
import {
  buildHermesRouterConsoleUrl,
  buildHermesRouterRouteFileName,
  buildHermesRouterRouteRecord,
} from './hermes-router-helpers.mjs'
...
function buildRuntimeUserName(prefix, instanceName) {
  const normalizedPrefix = sanitizeIdentifier(prefix, 'ocl', 10)
...
function buildServiceName(prefix, instanceName) {
  const normalizedPrefix = sanitizeIdentifier(prefix, 'hermes', 20)
...
function buildConsoleAccessUrl(config, instanceName, consolePort) {
  if (isRouterEnabled(config)) {
    return buildHermesRouterConsoleUrl(config.router.baseUrl, instanceName)
  }
...
function loadLocalAuthProfiles() {
  try {
    const authPath = join(homedir(), '.hermes', 'agents', 'main', 'agent', 'auth-profiles.json')
    return readFileSync(authPath, 'utf8')
  } catch (e) {
    try {
      const globalAuthPath = join(homedir(), '.hermes', 'auth-profiles.json')
...
function loadLocalHermesConfig() {
  try {
    const configPath = join(homedir(), '.hermes', 'hermes.json')
...
function buildModelProxyModelMap() {
  const configuredMap = parseJsonObject(process.env.HERMES_MODEL_PROXY_MODEL_MAP_JSON ?? '')
...
function buildGeneratedModelProxyProvider(plan) {
  const providerId =
    sanitizeIdentifier(process.env.HERMES_MODEL_PROXY_PROVIDER_ID ?? 'thousand-engine', 'thousand-engine', 40) ||
    'thousand-engine'
  const keyEnvId = 'HERMES_MODEL_PROXY_TOKEN'
  const upstreamKeyValue = String(process.env.QS_KEY ?? '').trim()
  const baseUrl = normalizeModelProxyUpstreamBaseUrl(process.env.HERMES_MODEL_PROXY_BASE_URL ?? '')
  ...
  return {
    ...
    providerConfig: {
      ...
      api: String(process.env.HERMES_MODEL_PROXY_API ?? 'openai-completions').trim() || 'openai-completions',
...
function getHermesConfigPath(workspacePath) {
  return `${workspacePath}/.hermes/hermes.json`
}

function getHermesStatePath(workspacePath) {
...
function buildRuntimePassthroughEnvironment() {
  const additionalKeys = String(process.env.HERMES_RUNTIME_ENV_PASSTHROUGH ?? '')
    .split(/[,\s]+/)
...
function buildPlan(config, context) {
  const localHermesConfig = loadLocalHermesConfig()
  const runtimeUser = buildRuntimeUserName(config.hermes.runtimeUserPrefix, context.instanceName)
  const serviceName = buildServiceName(config.hermes.servicePrefix, context.instanceName)
  const workspacePath = `${stripTrailingSlash(config.hermes.baseDir)}/instances/${context.instanceName}`
  ...
  const environment = {
    ...
    [config.hermes.modelEnvName]: context.order.model_id,
    [config.hermes.channelEnvName]: context.order.channel_id,
    [config.hermes.planEnvName]: context.order.plan_id,
    HERMES_INSTANCE_NAME: context.instanceName,
    HERMES_INSTANCE_ROOT: workspacePath,
    HERMES_PUBLIC_URL: publicEndpoint,
    HERMES_CONSOLE_URL: consoleUrl,
    HERMES_CONFIG_PATH: getHermesConfigPath(workspacePath),
    HERMES_STATE_DIR: getHermesStatePath(workspacePath),
    HERMES_GATEWAY_TOKEN: consoleToken,
    HERMES_CHANNEL_BOUND: hasCommunicationToken ? 'true' : 'false',
  }
...
function buildUpgradePlan(config, context) {
  const workspacePath = context.workspacePath || `${stripTrailingSlash(config.hermes.baseDir)}/instances/${context.instanceName}`
...
function buildLifecyclePlan(config, context) {
  ...
  const runtimeUser =
    context.runtimeUser ||
    buildRuntimeUserName(config.hermes.runtimeUserPrefix, context.instanceName)
  const serviceName =
    context.serviceName ||
    buildServiceName(config.hermes.servicePrefix, context.instanceName)
...
function renderHermesHomeConfig(plan) {
  const localConfig = loadLocalHermesConfig()
  const localModels =
  ...
  const renderedAgentDefaults = {
    workspace: `${plan.workspacePath}/.hermes/workspace`,
  }
...
function renderSystemdService(plan, config) {
  const startCommand = normalizeStartupCommand(config.hermes.startCommand)

  return `[Unit]
Description=Hermes instance ${plan.serviceName}
...
ExecStart=/bin/bash -lc ${shellEscape(`PATH=/usr/local/bin:/usr/bin:/bin:$HOME/.local/bin:$PATH ${startCommand}`)}
Restart=always
RestartSec=5
...
function requireRepositoryUrl(config) {
  const repoUrl = String(config?.hermes?.repoUrl ?? '').trim()
  if (!repoUrl) {
    const configPath = config?.path ? ` in ${config.path}` : ''
    throw new Error(
      `Hermes repository URL is missing${configPath}. Set hermes.repoUrl to a valid Git repository URL before deploying.`,
    )
  }
...
function requireArchiveUrl(config) {
  const archiveUrl = String(config?.hermes?.archiveUrl ?? '').trim()
  if (!archiveUrl) {
    const configPath = config?.path ? ` in ${config.path}` : ''
    throw new Error(
      `Hermes archive URL is missing${configPath}. Set hermes.archiveUrl to a valid tar.gz or zip package URL before deploying.`,
    )
  }
...
function readArchivePath(config) {
  return String(config?.hermes?.archivePath ?? '').trim()
}

function resolveDeploymentSource(config) {
  const sourceType = config?.hermes?.sourceType === 'archive' ? 'archive' : 'git'
  ...
  return {
    ...
    repoRef: String(config?.hermes?.repoRef ?? '').trim() || 'main',
  }
}
...
function describeDeploymentConfig(config) {
  return [
    ...
    `sourceType=${config?.hermes?.sourceType ?? 'git'}`,
    `repoUrl=${String(config?.hermes?.repoUrl ?? '').trim() || '<empty>'}`,
    `repoRef=${String(config?.hermes?.repoRef ?? '').trim() || '<empty>'}`,
    `archiveUrl=${String(config?.hermes?.archiveUrl ?? '').trim() || '<empty>'}`,
    `archivePath=${String(config?.hermes?.archivePath ?? '').trim() || '<empty>'}`,
...
function buildDeploymentFailureError(error, config, plan) {
  ...
  Object.assign(failure, {
    ...
    hermesVersion: plan?.targetVersion ?? config?.hermes?.repoRef ?? null,
  })
...
function renderAppSetupCommand(plan, source) {
  if (source.type === 'archive') {
    const stagedArchivePath = `${plan.workspacePath}/.tmp/hermes-template.tar.gz`
    return `set -euo pipefail
rm -rf ${shellEscape(plan.appPath)}
EXTRACT_DIR="$(mktemp -d ${shellEscape(`${plan.workspacePath}/.tmp/hermes-extract.XXXXXX`)})"
if [ -n ${shellEscape(source.archivePath ?? '')} ]; then
  ...
  ARCHIVE_FILE="$(mktemp ${shellEscape(`${plan.workspacePath}/.tmp/hermes-src.XXXXXX`)})"
  ...
  echo 'Neither a pre-uploaded archive path nor curl/wget download is available for the Hermes archive.' >&2
  exit 1
fi
if [ ! -f "$ARCHIVE_FILE" ]; then
  echo "Hermes archive not found at $ARCHIVE_FILE" >&2
  exit 1
fi
if tar -xzf "$ARCHIVE_FILE" -C "$EXTRACT_DIR" >/dev/null 2>&1; then
  ...
  echo 'Unable to extract the Hermes archive. Install unzip or provide a tar.gz package.' >&2
  exit 1
fi
...
if [ "$ENTRY_COUNT" = "0" ]; then
  echo 'Hermes archive was extracted, but no files were found.' >&2
  exit 1
fi
...
function renderRemoteScript(plan, config) {
  ...
  const hermesHomeConfig = Buffer.from(renderHermesHomeConfig(plan), 'utf8').toString('base64')
  ...
  const routeFilePath =
    isRouterEnabled(config) && plan.consolePort !== null
      ? `${stripTrailingSlash(config.router.routesDir)}/${buildHermesRouterRouteFileName(plan.instanceName)}`
      : ''

  const installCommand = config.hermes.installCommand || 'true'
  const buildCommand = config.hermes.buildCommand || 'true'
...
HERMES_HOME_CONFIG_B64=${shellEscape(hermesHomeConfig)}
...
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" "$WORKSPACE_PATH/.hermes"
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" "$WORKSPACE_PATH/.hermes/workspace"
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" "$WORKSPACE_PATH/.hermes/agents"
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" "$WORKSPACE_PATH/.hermes/agents/main"
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" "$WORKSPACE_PATH/.hermes/agents/main/agent"
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" ${shellEscape(getHermesStatePath(plan.workspacePath))}
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" ${shellEscape(`${getHermesStatePath(plan.workspacePath)}/agents`)}
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" ${shellEscape(`${getHermesStatePath(plan.workspacePath)}/agents/main`)}
install -d -m 700 -o "$RUNTIME_USER" -g "$RUNTIME_USER" ${shellEscape(`${getHermesStatePath(plan.workspacePath)}/agents/main/agent`)}
...
ARCHIVE_STAGE_PATH=${shellEscape(`${plan.workspacePath}/.tmp/hermes-template.tar.gz`)}
if [ ! -f "$ARCHIVE_SOURCE_PATH" ]; then
  echo "Hermes template archive not found at $ARCHIVE_SOURCE_PATH" >&2
  exit 1
fi
...
su -s /bin/bash "$RUNTIME_USER" -c "cd ${shellEscape(plan.workspacePath)} && printf '%s' \\\"$HERMES_HOME_CONFIG_B64\\\" | base64 -d > ${shellEscape(`${plan.workspacePath}/.hermes/hermes.json`)} && chmod 600 ${shellEscape(`${plan.workspacePath}/.hermes/hermes.json`)}"
if [ -n "$AUTH_PROFILES_B64" ]; then
  su -s /bin/bash "$RUNTIME_USER" -c "cd ${shellEscape(plan.workspacePath)} && printf '%s' \\\"$AUTH_PROFILES_B64\\\" | base64 -d > ${shellEscape(`${plan.workspacePath}/.hermes/agents/main/agent/auth-profiles.json`)} && chmod 600 ${shellEscape(`${plan.workspacePath}/.hermes/agents/main/agent/auth-profiles.json`)}"
  su -s /bin/bash "$RUNTIME_USER" -c "cd ${shellEscape(plan.workspacePath)} && printf '%s' \\\"$AUTH_PROFILES_B64\\\" | base64 -d > ${shellEscape(`${getHermesStatePath(plan.workspacePath)}/agents/main/agent/auth-profiles.json`)} && chmod 600 ${shellEscape(`${getHermesStatePath(plan.workspacePath)}/agents/main/agent/auth-profiles.json`)}"
fi
...
if ! command -v python3 >/dev/null 2>&1 || ! python3 -c 'import sys; sys.exit(0 if sys.version_info >= (3, 11) else 1)' 2>/dev/null; then
  echo "Python 3.11 or later is required to run Hermes Agent. Install it with: apt install python3.11 python3-pip" >&2
  exit 1
fi
...
if [ "$ACTIVE_STATE" != "active" ]; then
  echo "Hermes service $SERVICE_NAME did not become active (state: $ACTIVE_STATE)." >&2
...
if [ -n "$CONSOLE_PORT" ] && command -v curl >/dev/null 2>&1; then
  ...
  if [ "$CONSOLE_READY" != "1" ]; then
    echo "Hermes console on port $CONSOLE_PORT did not become reachable." >&2
    journalctl -u "$SERVICE_NAME" -n 60 --no-pager >&2 || true
    exit 1
  fi
fi
...
if [ ! -d "$APP_PATH/.git" ]; then
  echo "Hermes app repository not found at $APP_PATH" >&2
  exit 1
fi
...
printf '%s\\n' ${shellEscape(
    JSON.stringify({
      ...
      serviceName: plan.serviceName,
      hermesVersion: plan.targetVersion,
    }),
  )}
`
}
...
function isLocalDeploymentTarget(config) {
  if (String(process.env.HERMES_DEPLOY_RUN_LOCAL ?? '').trim().toLowerCase() === 'true') {
    return true
  }
    ...
    JSON.stringify(
      {
        ...
        channelId: context.order.channel_id,
        runtimeUser: plan.runtimeUser,
        hermesVersion: config.hermes.repoRef,
      },
      null,
      2,
    ),
...
export async function listConfiguredHermesVersions(config) {
  const repository = parseGithubRepository(config.hermes.repoUrl)
  if (!repository) {
    throw new Error('Hermes repository must be hosted on GitHub to browse available versions.')
  }
    ...
    const response = await fetch(
      `https://api.github.com/repos/${repository.owner}/${repository.repo}/tags?per_page=100&page=${page}`,
      {
        headers: {
          ...
          'User-Agent': 'Hermes-Launch',
        },
      },
    )

    if (!response.ok) {
      throw new Error(`Unable to load Hermes versions from GitHub (${response.status}).`)
    }
  ...
  const configuredRef = config.hermes.repoRef.trim()
...
export async function executeConfiguredDeployment(config, context) {
  ...
  try {
    if (!hasDeploymentServerCredentials(config)) {
      throw new Error(
        'Deployment server credentials are missing in the active environment file. Set HERMES_DEPLOY_ROOT_PASSWORD or HERMES_DEPLOY_PRIVATE_KEY_PATH.',
      )
    }
    ...
    return {
      ...
      hermesVersion: parsed.hermesVersion ?? config.hermes.repoRef,
...
export async function upgradeConfiguredDeployment(config, context) {
  ...
  if (!plan.runtimeUser || !plan.serviceName || !plan.workspacePath) {
    throw new Error('Hermes instance is missing runtime deployment metadata.')
  }
  ...
  return {
    ...
    runtimeUser: parsed.runtimeUser ?? plan.runtimeUser,
    serviceName: parsed.serviceName ?? plan.serviceName,
    hermesVersion: parsed.hermesVersion ?? plan.targetVersion,
    runLogs: `${result.stdout}${result.stderr}`.trim(),
  }
}
