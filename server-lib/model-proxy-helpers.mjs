...
export function buildInternalModelProxyBaseUrl(instanceName, environment = process.env) {
  const configuredBaseUrl = stripTrailingSlash(String(environment.HERMES_MODEL_PROXY_INTERNAL_BASE_URL ?? '').trim())
  const baseUrl =
...
export function buildModelProxyInternalToken(instanceName, environment = process.env) {
  const normalizedInstanceName = String(instanceName ?? '').trim()
  const secret = String(
    environment.HERMES_MODEL_PROXY_INTERNAL_SECRET ??
      environment.HERMES_CONFIG_SECRET ??
      environment.HERMES_TOKEN_SECRET ??
      '',
  ).trim()
...
export function isAllowedModelProxyRemoteAddress(address, environment = process.env) {
  ...
  const normalized = normalizeRemoteAddress(address)
  const configuredAddresses = String(environment.HERMES_MODEL_PROXY_ALLOWED_REMOTE_ADDRESSES ?? '')
    .split(',')
