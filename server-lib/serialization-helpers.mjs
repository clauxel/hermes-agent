export function createSerializationHelpers({
  ...
  function serializeClawInstance(row) {
    if (!row) {
      return null
    }
    ...
    return {
      ...
      hermesVersion: getClawHermesVersion(row),
      upgradeStatus: getClawUpgradeStatus(row),
  ...
  async function serializeOrder(row, { viewerContext = null } = {}) {
    ...
    const instance = serializeClawInstance(await findLatestClawInstanceByOrderIdStatement.get(row.id))
    ...
    const configuredHermesVersion = getConfiguredHermesVersion()
    const hermesVersion = instance?.hermesVersion ?? configuredHermesVersion
