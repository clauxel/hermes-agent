...
export function canHermesManagementConsole(row) {
  const hermesConsoleUrl = row.claw?.consoleUrl ?? row.deployment?.consoleUrl ?? null
  ...
  return Boolean(hermesConsoleUrl) && (clawRuntimeAvailable || deploymentReady)
}

export function buildHermesManagementRows(consoleData) {
...
export function buildConsoleMetrics(consoleData) {
  ...
  return {
    ...
    liveClaws: consoleData.claws.filter((claw) => claw.status === 'running').length,
    createdClaws: consoleData.claws.length,
